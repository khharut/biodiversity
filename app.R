...
ui <- fluidPage(
  tags$head(tags$link(rel = "stylesheet", type = "text/css", href = "sass.min.css")),
  sidebarPanel(
...
server <- function(input, output, session) {
  biodiversity_data <- read.csv("poland_biodiversity.csv", sep = ",", header = TRUE)
  variable_names <- c("longitudeDecimal", "latitudeDecimal", "vernacularName", "scientificName", "time")
  biodiversity_data$time <- paste(biodiversity_data$eventDate, biodiversity_data$eventTime, sep = " ")
  biodiversity_data$time <- as.POSIXct(biodiversity_data$time, format="%Y-%m-%d %H:%M")
  ...
  get_species_data_sc <-function(species) {
    ...
    if (length(species_loc) > 0) {
      ...
      species_data <- species_data[order(species_data$time, decreasing = FALSE),]
      species_data$time <- as.character(species_data$time)
    } else {
      species_data <- data.frame(matrix(ncol = length(variable_names), nrow = 0))
  ...
  get_random_sample <- function(n=-1) {
    ...
    random_sample <- random_sample %>% distinct(scientificName, .keep_all = TRUE)
  ...
  biodiversity_data_random <- get_random_sample()
  ...
  output$species_sc_selection <- renderUI(
          selectInput("sc_name", "Choose a species by scientific name",
                      choices = species_sc, selected = species_sc[1]))
  
  output$species_hm_selection <- renderUI(
          selectInput("hm_name", "Choose a species by human name",
                      choices = species_hm, selected = NULL))
